...
public final class MnemonicValidator {
    ...
    /**
     * Get a Mnemonic validator for the given word list. No normalization is
     * currently performed, this is an open issue:
     * https://github.com/NovaCrypto/BIP39/issues/13
     *
     ...
     * @throws InvalidChecksumException      If the last bytes don't match the
     *                                       expected last bytes
     * @throws InvalidWordCountException     If the number of words is not a
     *                                       multiple of 3, 24 or fewer
     * @throws WordNotFoundException         If a word in the mnemonic is not
     *                                       present in the word list
     * @throws UnexpectedWhiteSpaceException Occurs if one of the supplied words is
     *                                       empty, e.g. a double space
     */
    public byte[] validate(final CharSequence mnemonic) throws InvalidChecksumException, InvalidWordCountException,
            WordNotFoundException, UnexpectedWhiteSpaceException {
        return validate(charSequenceSplitter.split(mnemonic));
    }

    /**
     * Check that the supplied mnemonic fits the BIP0039 spec.
     * <p>
     * The purpose of this method overload is to avoid constructing a mnemonic
     * String if you have gathered a list of words from the user.
     *
     * @param mnemonic The memorable list of words
     * @throws InvalidChecksumException      If the last bytes don't match the
     *                                       expected last bytes
     ...
     *                                       empty
     */
    public byte[] validate(final Collection<? extends CharSequence> mnemonic) throws InvalidChecksumException,
            InvalidWordCountException, WordNotFoundException, UnexpectedWhiteSpaceException {
    ...
    private static byte[] validate(final int[] wordIndexes) throws InvalidWordCountException, InvalidChecksumException {
    ...
    private int[] findWordIndexes(final Collection<? extends CharSequence> split)
            throws UnexpectedWhiteSpaceException, WordNotFoundException {
