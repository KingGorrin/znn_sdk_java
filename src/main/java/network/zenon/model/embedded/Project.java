...
public class Project extends AcceleratorProject implements IJsonConvertible<JProject> {
    ...
    public Project(JProject json) {
    	...
    	this.phaseIds = Collections.unmodifiableList(
    			json.phaseIds.stream().map(x -> Hash.parse(x)).toList());
    	this.phases = Collections.unmodifiableList(
    			json.phases.stream().map(x -> new Phase(JPhase.fromJObject(x))).toList());
    ...
    public long getPaidZnnFunds() {
        return this.phases.stream()
        	.filter(x -> x.getStatus() == AcceleratorProjectStatus.PAID)
    ...
    public long getPendingZnnFunds() {
        ...
        if (lastPhase != null &&
            lastPhase.getStatus() == AcceleratorProjectStatus.ACTIVE) {
