...
public class SentinelApi {
    ...
    public SentinelInfoList getAllActive(int pageIndex, int pageSize) {
    	JSentinelInfoList response = this.client.sendRequest("embedded.sentinel.getAllActive", new Object[] { pageIndex, pageSize }, JSentinelInfoList.class);
        return new SentinelInfoList(response);
    }

    public SentinelInfo getByOwner(Address owner) {
    	JSentinelInfo response = this.client.sendRequest("embedded.sentinel.getByOwner", new Object[] { owner.toString() }, JSentinelInfo.class);
        return response != null ? new SentinelInfo(response) : null;
    }

    public long getDepositedQsr(Address address) {
        return this.client.sendRequest("embedded.sentinel.getDepositedQsr", new Object[] { address.toString() }, long.class);
    }

    public UncollectedReward getUncollectedReward(Address address) {
    	JUncollectedReward response = this.client.sendRequest("embedded.sentinel.getUncollectedReward", new Object[] { address.toString() }, JUncollectedReward.class);
    ...
    public RewardHistoryList getFrontierRewardByPage(Address address, int pageIndex, int pageSize) {
    	JRewardHistoryList response = this.client.sendRequest("embedded.sentinel.getFrontierRewardByPage", new Object[] { address.toString(), pageIndex, pageSize }, JRewardHistoryList.class);
